...
Твоя задача — выбрать из этого списка одежду, которая лучше всего подходит под запрос. Образ должен сочетаться друг с другом и запросом, а так же иметь хотя бы 1 тип верхней одежды, штанов и обуви.
Верни только список ID выбранных вещей, а также примерные координаты для расположения на холсте (0–100% по X и Y).
